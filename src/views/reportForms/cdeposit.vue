...
export default {
  ...
  setup() {
    ...
    const channgeType = (item, index) => {
      ...
      state.currentcode = item.platformCode;
      state.rechargeType = item.rechargeType;
      data.value = item;
    };
    ...
    const open = () => {
      // window.location.href = 'http://apps.apple.com/cn/app/id387682726'; // 打开淘宝
      // $router.push('/reportForms/transfer');
    };
      ...
      reportforms
        ...
        .then((res) => {
          if (res && res.code === 200) {
            ...
            state.currentcode = res.data[0].platformCode;
            state.rechargeType = res.data[0].rechargeType;
